...
with open('qiwi_symphony_payments.csv', 'wb') as csvfile:
    ...
    cursor.execute("select date(pay.created_at) as received, substring(sub.shopper_reference,12,15) as order_id, pay.psp_reference as reference, pay.currency, round(pay.amount/100),sub.billing_period as package, sub.selected_brand as provider, pay.ip as ip, sub.user_id as uid, 'Qiwi Wallet' as payment_type, case when sub.cancelled_at is not null then true else false end as cancelled,null as cancelled_at, status from sfbusuudata.adyen_payment pay inner join sfbusuudata.adyen_subscription sub on sub.id = pay.subscription_id;")
    data = cursor.fetchall()
...
cursor.execute("create table symphony_payments2( received varchar(15), order_id varchar(100), reference varchar(100), currency varchar(5), amount decimal, billing_period varchar(30), provider varchar(20), ip varchar(30), uid int, payment_method varchar(50), cancelled boolean, cancelled_at timestamp, status varchar(max)); ")
